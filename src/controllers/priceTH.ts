...
export const priceTHHandler = async () => {
  const response = await fetch(PRICE_CONFIG.API_TH_URL);
  ...
  const priceComparePrevious = $(PRICE_CONFIG.SELECTOR.PRICE_COMPARE_PREVIOUS).text().trim();
  const priceCompareYesterday = $(PRICE_CONFIG.SELECTOR.PRICE_COMPARE_YESTERDAY).text().trim().substring(7);

  const formatPriceComparePrevious =
    priceComparePrevious.charAt(0) === "-" ? priceComparePrevious : `+${priceComparePrevious}`;
  const formatPriceCompareYesterday =
    priceCompareYesterday.charAt(0) === "-" ? priceCompareYesterday : `+${priceCompareYesterday}`;
